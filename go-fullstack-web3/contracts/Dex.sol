...
contract Dex {
	...
	function swap(
		...
		ERC20 baseToken = pools[poolName].poolTokensSwapped[quoteToken.symbol()];

		uint256 baseTokenToReturn = calculateTokenSwap(
			poolName,
			quoteToken,
			depositAmount
		);
		...
		require(
			baseToken.balanceOf(address(this)) > baseTokenToReturn,
			"not enough base tokens in the pool"
		);

		quoteToken.transferFrom(msg.sender, address(this), depositAmount);

		baseToken.approve(address(this), baseTokenToReturn);
		baseToken.transferFrom(address(this), msg.sender, baseTokenToReturn);
	}
	...
	function getPrice(string calldata poolName, ERC20 baseToken)
		...
		returns (uint256)
	{
		ERC20 quoteToken = pools[poolName].poolTokensSwapped[baseToken.symbol()];

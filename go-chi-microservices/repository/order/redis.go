...
import (
	...
	"github.com/zeindevs/go-chi-microservices/model"
)
...
func (r *RedisRepo) DeleteByID(ctx context.Context, id uint64) error {
	...
	if err := r.Client.Del(ctx, key).Err(); errors.Is(err, redis.Nil) {
		txn.Discard()
		return ErrNotExist
	} else if err != nil {
		txn.Discard()
...
func (r *RedisRepo) FindAll(ctx context.Context, page FindAllPage) (FindResult, error) {
	...
	if err != nil {
		return FindResult{}, fmt.Errorf("failed to got orders: %w", err)
	}
